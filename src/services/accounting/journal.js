...
// Serialize concurrent journal writes per (tenant, ref_type, ref_id).
// Prevents duplicate rows when fire-and-forget from upsertPayment races with
// a Rebuild Journal call for the same document.
const _queue = new Map();
function serializeWrite(key, fn) {
  const prev = _queue.get(key) || Promise.resolve();
  const next = prev.then(fn, fn); // always run fn even if prev errored
  _queue.set(key, next.catch(() => {}));
  return next;
}
...
function toIsoDate(str) {
  ...
  // ISO YYYY-MM-DD — return as-is; parseInvoiceDate expects DD-MM-YYYY and would mangle it
...
async function postInvoice(req, invoice) {
  try {
    ...
    await serializeWrite(`${tenant}|${ref_type}|${ref_id}`, async () => {
...
async function postPayment(req, payment) {
  try {
    ...
    await serializeWrite(`${tenant}|${ptype}|${pid}`, async () => {
...
async function postExpense(req, expense) {
  try {
    ...
    await serializeWrite(`${tenant}|expense|${id}`, async () => {

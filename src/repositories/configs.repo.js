...
// Merge-save: only override the given fields, preserve everything else.
async function patchSellerProfile(req, fields, targetUserId) {
  const existing = await getSellerProfile(req, targetUserId);
  await saveSellerProfile(req, { ...existing, ...fields }, targetUserId);
}

module.exports = { getSellerProfile, saveSellerProfile, patchSellerProfile, getMasterConfigProfile };

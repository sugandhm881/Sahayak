  ...
  try {
    ...
    for (const inv of invoices) {
      ...
      if (cat === 'sale' && !is_cn && dtype === 'invoice') {
        ...
        if (inv_date) monthly_trend[inv_month_label] = (monthly_trend[inv_month_label] || 0) + amt;
        const cname = inv.client_name || 'Unknown';
        top_clients[cname] = (top_clients[cname] || 0) + amt;
    ...
    try {
      const out = await reports.computeOutstanding(req);

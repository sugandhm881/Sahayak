...
// Paginated + filtered — used by new views. Existing /invoices-list untouched.
router.get('/v2/invoices-list', loginRequired, async (req, res) => {
  try {
    const { category, from, to, page = '1', limit = '50' } = req.query;
    const result = await loadInvoicesFiltered(req, {
...
router.post('/generate-invoice', loginRequired, perMinute(30), async (req, res) => {
  try {
    ...
    const client_details = {
      ...
      // shipto_* included so explicit Ship To values update the vendor record;
      // saveSingleClient ignores empty shipto fields to protect saved shipping addresses.
      shipto_address1: data.shipto_address1 || '',
      shipto_address2: data.shipto_address2 || '',
      shipto_pincode:  data.shipto_pincode  || '',
      shipto_district: data.shipto_district || '',
      shipto_state:    data.shipto_state    || '',
      shipto_name:     data.shipto_name     || '',
      shipto_mobile:   data.shipto_mobile   || '',
      shipto_email:    data.shipto_email    || '',
      shipto_gstin:    data.shipto_gstin    || '',
    };
    ...
    for (let i = 0; i < particulars.length; i++) {
      ...
      // Spread existing_data first so any future fields (product_id, unit, barcode, etc.)
      // are always preserved — only explicitly override the invoice-relevant fields.
      await saveSingleParticular(req, final_save_name, {
        ...existing_data,
        hsn: is_non_gst ? '' : (hsns[i] || existing_data.hsn || ''),
        rate: rates[i] || existing_data.rate || 0,
        taxrate: is_non_gst ? 0 : (taxrates[i] || existing_data.taxrate || 0),
    ...
    if ((data.auto_generate === undefined || data.auto_generate) && !is_edit) {
      ...
      invoice_date_str = formatLedgerDate(data.manual_invoice_date || data.invoice_date) || formatDDMonYYYY();
    }
    ...
    if (!is_edit) {
      ...
      if (direction !== 0) {
        ...
        // Queue Easyecom adjustment (never blocks the invoice save)
        try {
          const ecProfile = await getSellerProfile(req);
          const _ecKey = (ecProfile.easyecom_api_key || '').trim() || (process.env.EASYECOM_API_KEY || '').trim();
          if (_ecKey) {
            await easyecomService.queueAndMaybePush(req, ecProfile, particulars, qtys, direction, bill_no);
          }
        } catch (e) {
          console.error('[Easyecom] queue on invoice create failed:', e.message);
        }
      }
    }
...
router.delete('/delete-invoice/:bill_no(*)', loginRequired, async (req, res) => {
  try {
    ...
    // Reverse Easyecom adjustment before deletion
    try {
      const ecProfile = await getSellerProfile(req);
      const _ecKey2 = (ecProfile.easyecom_api_key || '').trim() || (process.env.EASYECOM_API_KEY || '').trim();
      if (_ecKey2) {
        const row = await getDocumentRow(req, billNo);
        if (row && row.row) {
          const inv = row.row.data || {};
          const dc = inv.doc_category, dt = inv.doc_type;
          let dir = 0;
          if (dc === 'purchase') dir = dt === 'grn' ? 1 : (dt === 'dn' ? -1 : 0);
          else if (dc === 'sale') dir = dt === 'invoice' ? -1 : (dt === 'cn' ? 1 : 0);
          if (dir !== 0) {
            // Reverse direction: restores what was originally deducted/added
            await easyecomService.queueAndMaybePush(req, ecProfile, inv.particulars || [], inv.qtys || [], -dir, `DEL:${billNo}`);
          }
        }
      }
    } catch (e) {
      console.error('[Easyecom] reverse on delete failed:', e.message);
    }

    // Reverse local inventory before deletion
    try {
      const row2 = await getDocumentRow(req, billNo);
      if (row2 && row2.row) {
        const inv2 = row2.row.data || {};
        const dc2 = inv2.doc_category, dt2 = inv2.doc_type;
        let dir2 = 0;
        if (dc2 === 'purchase') dir2 = dt2 === 'grn' ? 1 : (dt2 === 'dn' ? -1 : 0);
        else if (dc2 === 'sale') dir2 = dt2 === 'invoice' ? -1 : (dt2 === 'cn' ? 1 : 0);
        if (dir2 !== 0) {
          const ts2 = nowIso();
          const parts2 = inv2.particulars || [];
          const qtys2  = inv2.qtys || [];
          for (let k = 0; k < parts2.length; k++) {
            const iname = parts2[k];
            const iqty  = parseFloat(qtys2[k] || 0);
            if (!iname || iqty <= 0) continue;
            const sid = safeItemId(iname);
            if (!sid) continue;
            const existing = await getProduct(req, sid);
            const cur = existing ? parseFloat(existing.current_stock || 0) : 0;
            const restored = cur + iqty * (-dir2);
            await upsertProduct(req, sid, { current_stock: restored, last_updated: ts2 });
            await addLedgerEntry(req, {
              ref_doc_no: `DEL:${billNo}`, date: ts2.slice(0, 10),
              doc_type: 'cancellation', item_name: iname,
              qty_change: iqty * (-dir2), running_balance: restored, timestamp: ts2,
            });
          }
        }
      }
    } catch (e) {
      console.error('[Inventory] reverse on delete failed:', e.message);
    }
